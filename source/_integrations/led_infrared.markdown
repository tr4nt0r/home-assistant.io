...
If your LED Infrared setup is not working as expected, first make sure the infrared transmitter and receiver are correctly connected and that the configured entities are still available in Home Assistant. If you are using an infrared receiver, verify that your remote is sending signals that Home Assistant can detect.
